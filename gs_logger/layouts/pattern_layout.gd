...
func build(message: Message, format: int):
	match format:
		Logger.LOG_FORMAT_DEFAULT:
			return (
				"%-10s %-8d %s"
				% [Logger.get_level_name(message.level), message.line, message.text]
			)
		Logger.LOG_FORMAT_FULL:
			return (
				"%s %-8s %-8s %-8d %s"
				% [
					Logger.get_formatted_date(OS.get_datetime()),
					message.category.to_upper(),
					Logger.get_level_name(message.level),
					message.line,
					message.text
				]
			)
